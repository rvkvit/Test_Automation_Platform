...
def dashboard():
    ...
    if current_user.has_role('Admin'):
        # Admin can see all users and projects
        users = User.query.order_by(User.username).all()
        projects = Project.query.order_by(Project.name).all()
        pending_invitations = InvitationToken.query.filter(
        ...
        # Regular users can only see their own projects and team members
        owned_projects = Project.query.filter_by(owner_id=current_user.id).all()
        
        # Get all users from owned projects
        ...
        # Only invitations created by current user
        pending_invitations = InvitationToken.query.filter(
